  ...
  always @(posedge clk) begin
    ...
    if (!en) begin
      ...
      if (position_bomb[0] == 127 && player_not_on_bomb) begin
        ...
        start_bomb[0] <= 1;
      end else if (bomb_limit > 1 && position_bomb[1] == 127 && player_not_on_bomb) begin
        position_bomb[1] <= player_index;
        start_bomb[1] <= 1;
      end else if (bomb_limit > 2 && position_bomb[2] == 127 && player_not_on_bomb) begin
        position_bomb[2] <= player_index;
        start_bomb[2] <= 1;
      end
    end
    if (other_position_bomb[0] != other_position_bomb_i[6:0] || other_position_bomb[1] != other_position_bomb_i[13:7] || other_position_bomb[2] != other_position_bomb_i[20:14]) begin
      other_position_bomb[0] <= other_position_bomb_i[6:0];
      other_position_bomb[1] <= other_position_bomb_i[13:7];
      other_position_bomb[2] <= other_position_bomb_i[20:14];
      ...
      if (other_position_bomb[2] != 127) begin
        start_bomb[5] <= 1;
      end
    end
    ...
    if (explosion_display_count > 0) begin
    ...
    end else if (explode_bomb || e_explode_bomb) begin
      ...
      if (explode_bomb[0]) begin
        ...
        start_bomb[3] <= 0;
      end else if (e_explode_bomb[1]) begin
        bomb_index = other_position_bomb[1];
        start_bomb[4] <= 0;
      end else if (e_explode_bomb[2]) begin
        bomb_index = other_position_bomb[2];
        start_bomb[5] <= 0;
      end else bomb_index = 127;
      ...
      explode_up = '{
          7'(bomb_index - 3 * MAX_GRID_COLUMN),
          7'(bomb_index - 2 * MAX_GRID_COLUMN),
          7'(bomb_index - MAX_GRID_COLUMN)
      };
      ...
      explode_left_constraint = {
        (bomb_range > 2 & explode_left[2] / 12 == bomb_index_y),
        (bomb_range > 1 & explode_left[1] / 12 == bomb_index_y),
        (explode_left[0] / 12 == bomb_index_y)
      };
      explode_right_constraint = {
        (bomb_range > 2 & explode_right[2] / 12 == bomb_index_y),
        (bomb_range > 1 & explode_right[1] / 12 == bomb_index_y),
        (explode_right[0] / 12 == bomb_index_y)
      };
    ...
    if (push_bomb_ability && player_index != previous_player_index && (player_index == position_bomb[0] || player_index == position_bomb[1] || player_index == position_bomb[2])) begin
      ...
      /* Store calculations*/
      bomb_indices = '{
          7'(position_bomb[2] + bomb_offset),
          7'(position_bomb[1] + bomb_offset),
          7'(position_bomb[0] + bomb_offset)
      };
      bomb_indices_x = '{
          6'(bomb_indices[2] % 12),
          6'(bomb_indices[1] % 12),
      ...
      bomb_wall_collision = {
        after_break_tiles[bomb_indices[2]] == 0 & wall_tiles[bomb_indices[2]] == 0,
        after_break_tiles[bomb_indices[1]] == 0 & wall_tiles[bomb_indices[1]] == 0,
        after_break_tiles[bomb_indices[0]] == 0 & wall_tiles[bomb_indices[0]] == 0
      };
      bomb_screen_collision = {
        (bomb_indices_x[2] == 0 | bomb_indices_x[2] == 11),
        (bomb_indices_x[1] == 0 | bomb_indices_x[1] == 11),
        (bomb_indices_x[0] == 0 | bomb_indices_x[0] == 11)
      };
      bomb_constraint = {
        bomb_indices[2] < 96 & !bomb_screen_collision[2] & bomb_wall_collision[2],
        bomb_indices[1] < 96 & !bomb_screen_collision[1] & bomb_wall_collision[1],
        bomb_indices[0] < 96 & !bomb_screen_collision[0] & bomb_wall_collision[0]
      };
      ...
      if (player_index == position_bomb[0] && bomb_constraint[0]) begin
        ...
        end else if (bomb_indices[0] == position_bomb[2] && bomb_constraint[2]) begin
        ...
        end else position_bomb[0] <= bomb_indices[0];
      end else if (player_index == position_bomb[1] && bomb_constraint[1]) begin
        ...
        end else if (bomb_indices[1] == position_bomb[2] && bomb_constraint[2]) begin
        ...
        end else position_bomb[1] <= bomb_indices[1];
      end else if (player_index == position_bomb[2] && bomb_constraint[2]) begin
        ...
        end else if (bomb_indices[2] == position_bomb[1] && bomb_constraint[1]) begin
        ...
        end else position_bomb[2] <= bomb_indices[2];
      end
    end
  end
endmodule

module time_bomb_explosion (
    input clk,
    start,
    input [13:0] bomb_time,
    output reg bomb_exploded = 0
);
